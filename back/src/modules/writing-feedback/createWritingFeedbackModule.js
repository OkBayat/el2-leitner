...
import { WritingFeedbackWorker } from "../../application/writing-feedback/WritingFeedbackWorker.js";
...
export function createWritingFeedbackModule({ pool, config = {}, adapters = {}, logger = console }) {
  ...
  const provider = enabled ? adapters.writingFeedbackProvider ?? new OllamaWritingFeedbackProvider({
  ...
  const worker = new WritingFeedbackWorker({ repository, provider, enabled, clock, idFactory, timeoutMs: config.timeoutMs, pollIntervalMs: enabled ? 1000 : 60_000, logger });
  const service = new WritingFeedback({
    ...
    cancelWork: (id) => worker.cancel(id),
  });
  return {
    service,
    worker,
