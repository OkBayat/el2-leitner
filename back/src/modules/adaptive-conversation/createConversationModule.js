...
import { OpenAIConversationProvider } from '../../infrastructure/ai/OpenAIConversationProvider.js';
...
export function createConversationModule({ pool, config, adapters = {}, sharedClient, synthesizeSpeech, logger = console, cancelWork = () => {}, cancelSessionWork = () => {} }) {
  ...
  const provider = enabled ? adapters.conversationProvider ?? new OpenAIConversationProvider({ client: sharedClient }) : null;
  ...
  recordings = new ConversationRecordings({ sessions, repository, idFactory, evaluationProfile: provider?.getIdentity() ?? null, cancelWork, logger,
    speech: adapters.conversationSpeech ?? (enabled ? new HttpSpeechRecognizer({ url: config.shadowing?.url, privateOnly: true }) : null),
  });
  audio = new ConversationQuestionAudio({ sessions, options: config.tts, logger,
    synthesizeSpeech: adapters.conversationSpeechSynthesis ?? synthesizeSpeech,
  });
