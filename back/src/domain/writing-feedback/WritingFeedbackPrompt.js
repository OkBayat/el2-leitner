...
import { MAXIMUM_WRITING_WORDS, parseWritingFeedbackTask } from './WritingFeedbackTask.js';

export const WRITING_FEEDBACK_PROMPT_VERSION = 'vocora-writing-feedback-v2';
...
For sentence and paragraph practice, include ielts_band in not_assessed and set ielts_band to null.
Only for task1-chart, task1-process, task2-essay or general-letter with sufficient evidence may you provide a cautious IELTS band estimate in half-band steps.
For insufficient_evidence, set ielts_band to null and include it in not_assessed for every mode.
For those full IELTS modes, apply the relevant task response or achievement, coherence, lexical resource, and grammar criteria.
Never return rewards, mastery or pass/fail.
...
export function buildWritingFeedbackMessages({ draftText, draftVersion, taskContext, contentVersion, locale }) {
  ...
  if (draftText.trim().split(/\s+/u).length > MAXIMUM_WRITING_WORDS) {
    throw new WritingFeedbackError('WRITING_FEEDBACK_INPUT_TOO_LARGE');
  }
