...
function conversationError(error) {
  ...
  if (error instanceof AiProviderError) return new ConversationError(error.code.replace('AI_PROVIDER_', 'CONVERSATION_'));
...
export class OpenAIConversationProvider {
  ...
  getIdentity() {
    return {
      schemaVersion: CONVERSATION_SCHEMA_VERSION, promptVersion: CONVERSATION_PROMPT_VERSION,
  ...
  async evaluate(input) {
    ...
    if (context.transcript.status === 'insufficient_evidence') {
      return { result: validateConversationResult({ schemaVersion: CONVERSATION_SCHEMA_VERSION,
        ...
        nextQuestion: context.currentQuestion, endConversation: false,
        notAssessed: [...CONVERSATION_NOT_ASSESSED] }, context), identity, metrics: {} };
    }
    const [system, learnerData] = buildConversationMessages(context);
    try {
      const { decoded, provenance, metrics } = await this.client.infer({
        instructions: system.content, input: learnerData.content,
        format: CONVERSATION_SCHEMA, formatName: 'vocora_speaking_turn',
        maxOutputTokens: 450, signal: input.signal,
      });
