...
import { AiEvaluationWorker } from '../src/application/ai-evaluation/AiEvaluationWorker.js';
...
it('subtracts database time from the persisted lease before granting provider execution and cleanup time',(t)=>{
 ...
 return (async()=>{
  ...
  const worker=new AiEvaluationWorker({clock:()=>new Date(now),timeoutMs:120000,cleanupTimeoutMs:5000,idFactory:()=> 'lease',repository:{
