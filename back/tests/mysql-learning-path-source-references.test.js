...
test("source reference adapter implements the read port and resolves published collection sections", async () => {
  ...
  const collection = await repository.findCollectionByPublicId("cambridge-vocabulary-for-ielts");
  ...
  assert.deepEqual(pool.calls[0].parameters, ["cambridge-vocabulary-for-ielts"]);
