...
CREATE TABLE IF NOT EXISTS collection_entries (
  ...
  active_vocabulary_entry_id BIGINT UNSIGNED
    GENERATED ALWAYS AS (CASE WHEN removed_at IS NULL THEN vocabulary_entry_id ELSE NULL END) STORED,
  created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (id),
  UNIQUE KEY collection_entries_public_id_unique (public_id),
  UNIQUE KEY collection_entries_active_unique (collection_id, active_vocabulary_entry_id),
