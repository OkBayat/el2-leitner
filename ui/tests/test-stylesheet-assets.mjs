...
for (const page of pages) {
  ...
  assert.equal(
    new Set(cleanHrefs).size,
    cleanHrefs.length,
    `${page} must not load the same stylesheet more than once.`
  );
  assert.equal(
    cleanHrefs.at(-1),
    "material3.css",
    `${page} must load the presentation layer last so one system owns visual states.`
  );
  ...
  for (const href of hrefs) {
    ...
    assert.ok(
      assetPath.startsWith(`${uiRoot}${path.sep}`),
      `${page} stylesheet must stay inside the UI root: ${href}`
    );
