...
assert.equal(window.VocoraRemediationKeyboardGuard.install(document, window), false,
  'Installing the guard twice must be idempotent.');
assert.equal(window.VocoraRemediationKeyboardGuard.loadReviewSessionUx(document), false,
  'The review UX loader must be idempotent.');
assert.match(
  document.querySelector('#vocora-review-session-ux-script')?.getAttribute('src') || '',
  /^review-session-ux\.js\?v=\d+$/,
  'Review UX must bypass stale browser/CDN cache entries.'
);
assert.doesNotMatch(guardSource, /vocoraRemediationPreviewContinue/,
  'The guard must not depend on a duplicated remediation Continue button.');
...
// Deferred ownership is read from the controller snapshot, so it still works if
// a DOM event was missed or the review coordinator loaded after remediation began.
deferred = true;
...
assert.equal(legacyNextCards, 0, 'Deferred remediation must not be bypassed by the legacy shortcut.');
assert.equal(unrelatedVoiceStarts, 0);
