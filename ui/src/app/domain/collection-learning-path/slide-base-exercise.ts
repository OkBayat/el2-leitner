...
export function parseSlideBaseExerciseSlides(
    configValue: unknown,
): readonly SlideBaseExerciseSlide[] {
    ...
    const slides = config['slides'].map((value) => {
        ...
        if (!source || !id || !type) {
            throw new Error(
                'Every slide-base exercise slide requires an id and type.',
            );
        }
        return {
            id,
            ...
            rootSlideId:
                String(source['rootSlideId'] ?? '').trim() || undefined,
    ...
    if (terminalSlides.length !== 1 || slides.at(-1)?.terminal !== true) {
        throw new Error(
            'Slide-base exercise requires exactly one terminal last slide.',
        );
    }
    return slides;
}
