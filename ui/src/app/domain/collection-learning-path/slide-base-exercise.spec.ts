...
describe('slide-base exercise definition', () => {
    ...
    it('rejects definitions without exactly one terminal last slide', () => {
        expect(() =>
            parseSlideBaseExerciseSlides({
                slides: [{ id: 'scope', type: 'message' }],
            }),
        ).toThrow(/terminal/iu);
        expect(() =>
            parseSlideBaseExerciseSlides({
    ...
    it('preserves legacy slide-base diagnostics', () => {
        expect(() => parseSlideBaseExerciseSlides({})).toThrow(
            'Slide-base exercise slides are unavailable.',
        );
        expect(() =>
            parseSlideBaseExerciseSlides({
                slides: [{ id: '', type: 'message', terminal: true }],
            }),
        ).toThrow('Every slide-base exercise slide requires an id and type.');
    });
});
