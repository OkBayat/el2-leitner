...
export class LibraryPageComponent implements OnInit {
  ...
  readonly coverMonogram = libraryCoverMonogram;
  ...
  markCoverMissing(slug: string): void {
    this.missingCoverSlugs.update((current) => {
  ...
  async open(collection: LibraryCollection): Promise<void> {
    const detail = await this.api.get(collection.id);
    const changed = await firstValueFrom(this.dialog.open(LibraryDetailDialogComponent, {
      data: {collection: detail.collection, canManage: Boolean(detail.capabilities?.canManage)},
      maxWidth: '95vw',
    }).afterClosed());
  ...
  async createCollection(): Promise<void> {
    const value = await firstValueFrom(this.dialog.open(CollectionEditorComponent, {data: {collection: null}}).afterClosed()) as CollectionPayload | undefined;
    if (!value) return;
    const result = await this.api.create(value);
    await this.load();
    this.snack.open('Collection created.', 'OK', {duration: 2000});
    await this.open(result.collection);
  }

  async edit(collection: LibraryCollection): Promise<void> {
    const value = await firstValueFrom(this.dialog.open(CollectionEditorComponent, {data: {collection}}).afterClosed()) as CollectionPayload | undefined;
    if (!value) return;
    await this.api.update(collection.id, value);
    await this.load();
  }
}
