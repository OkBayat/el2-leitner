...
import { accuracy, getDueWords, hardWords, isActiveLeitnerWord, localDay, totalStats } from '../../domain/learning/learning-rules';
...
@Component({
  ...
  template: `@if(state(); as s){<section class="page"><header><p>Today's plan</p><h1>{{ dueCount() ? dueCount() + ' reviews are waiting for you' : "Today's plan is complete!" }}</h1><p>Complete due reviews and new words in one short session.</p><div class="actions"><button mat-flat-button (click)="goReview()" [disabled]="!dueCount()">Start today's review</button><button mat-stroked-button (click)="startBoxOne()">Free practice: House 1</button><button mat-stroked-button (click)="addNewWords()">Add new words</button></div></header><div class="stats">@for(card of statCards(); track card.label){<mat-card appearance="outlined"><mat-card-subtitle>{{ card.label }}</mat-card-subtitle><mat-card-title>{{ card.value }}</mat-card-title><span>{{ card.unit }}</span></mat-card>}</div><div class="grid"><mat-card appearance="outlined"><mat-card-header><mat-card-title>14-day activity</mat-card-title></mat-card-header><mat-card-content class="bars">@for(day of activity(); track day.day){<div class="bar-column"><span>{{ day.value }}</span><i [style.height.%]="day.height"></i><small>{{ day.label }}</small></div>}</mat-card-content></mat-card><mat-card appearance="outlined"><mat-card-header><mat-card-title>House status</mat-card-title></mat-card-header><mat-card-content>@for(house of houses(); track house.box){<button class="house-row" type="button" (click)="openHouse(house.box)"><span>House {{ house.box }}</span><mat-progress-bar mode="determinate" [value]="house.percent"/><strong>{{ house.count }}</strong></button>}</mat-card-content></mat-card></div><mat-card appearance="outlined"><mat-card-header><mat-card-title>Needs more attention</mat-card-title><span class="spacer"></span><button mat-button (click)="router.navigateByUrl('/words')">View all</button></mat-card-header><mat-card-content>@if(hardest().length){@for(word of hardest(); track word.id){<div class="hard-word"><strong>{{ word.term }}</strong><span>{{ word.mistakes }} mistakes · {{ word.attempts }} attempts</span></div>}}@else{<p>No mistakes recorded yet.</p>}</mat-card-content></mat-card></section>}`,
  styles: [`:host{display:block}.page{display:grid;gap:20px}.page>header{padding:32px;border-radius:28px;background:var(--mat-sys-primary-container);color:var(--mat-sys-on-primary-container)}header h1{font-size:clamp(26px,4vw,42px);margin:6px 0}.actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:22px}.stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.stats mat-card{padding:18px}.stats mat-card-title{font-size:30px}.grid{display:grid;grid-template-columns:1.2fr .8fr;gap:18px}.grid mat-card{padding:18px}.bars{display:flex;height:170px;align-items:end;gap:6px}.bar-column{display:grid;flex:1;height:100%;align-content:end;text-align:center;gap:3px}.bar-column i{display:block;min-height:3px;background:var(--mat-sys-primary);border-radius:8px}.bar-column small{font-size:9px;color:var(--mat-sys-on-surface-variant)}.house-row{width:100%;display:grid;grid-template-columns:70px 1fr 30px;gap:10px;align-items:center;border:0;background:transparent;padding:10px;cursor:pointer;text-align:left;color:inherit;border-radius:12px}.house-row:hover{background:var(--mat-sys-surface-container)}.hard-word{display:flex;justify-content:space-between;padding:12px 0;border-bottom:1px solid var(--mat-sys-outline-variant)}.spacer{flex:1}@media(max-width:900px){.stats{grid-template-columns:1fr 1fr}.grid{grid-template-columns:1fr}}`],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DashboardPageComponent implements OnInit {
  ...
  readonly houses = computed(() => {
    const words = this.state()?.words.filter(isActiveLeitnerWord) || [];
    const counts = [1,2,3,4,5].map((box) => words.filter((word) => word.box === box).length);
    const max = Math.max(...counts, 1);
    return counts.map((count, index) => ({ box: index + 1, count, percent: Math.round(count / max * 100) }));
  });
  ...
  openHouse(box: number): void { void this.router.navigate(['/leitner-house', box]); }
