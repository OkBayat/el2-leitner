  ...
  template: `
    ...
    <mat-dialog-content class="story-dialog">
      <mat-button-toggle-group [value]="kind()" (change)="selectKind($event.value)">
        <mat-button-toggle value="daily">امروز</mat-button-toggle>
        <mat-button-toggle value="journey">مسیر کلی</mat-button-toggle>
      </mat-button-toggle-group>
      <div class="preview"><canvas #canvas width="1080" height="1920" aria-label="پیش‌نمایش استوری Vocora"></canvas></div>
      <p class="privacy">استوری فقط آمار کلی را نشان می‌دهد؛ ایمیل، پاسخ تایپ‌شده و نام واژه‌های خطادار منتشر نمی‌شوند.</p>
    </mat-dialog-content>
    <mat-dialog-actions align="end">
      <button mat-button (click)="copyCaption()">کپی متن</button>
      <button mat-stroked-button (click)="download()">دانلود PNG</button>
      <button mat-flat-button (click)="share()">اشتراک‌گذاری</button>
      <button mat-button (click)="dialog.close()">بستن</button>
    </mat-dialog-actions>
  `,
  styles: [`
...
export class ShareStoryDialogComponent implements AfterViewInit {
  ...
  private render(): void {
    ...
    ctx.fillStyle = '#1d1b20'; ctx.textAlign = 'center'; ctx.direction = 'rtl';
    ...
    ctx.font = '500 30px sans-serif'; ctx.fillText('vocora.app · تمرین واقعی، پیشرفت قابل اندازه‌گیری', 540, 1740);
  }
  ...
  private toBlob(): Promise<Blob> { return new Promise((resolve, reject) => this.canvasRef.nativeElement.toBlob((blob) => blob ? resolve(blob) : reject(new Error('ساخت تصویر انجام نشد.')), 'image/png')); }
  async copyCaption(): Promise<void> { await navigator.clipboard.writeText(this.moment().caption); this.snack.open('متن پیشنهادی کپی شد.', 'باشه', { duration: 2200 }); }
