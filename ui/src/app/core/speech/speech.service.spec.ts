...
describe('SpeechService playback events', () => {
  ...
  it('uses native word boundaries when the browser provides them', async () => {
    ...
    expect(service.speak('Hello world', 0.85, { onStart, onWordBoundary, onEnd })).toBe(true);
    const utterance = speech.utterance();
    utterance.onstart?.({});
    utterance.onboundary?.({ name: 'word', charIndex: 6, charLength: 5 });
    utterance.onboundary?.({ name: 'sentence', charIndex: 0, charLength: 11 });
    await vi.advanceTimersByTimeAsync(2_000);
    utterance.onend?.({});

    expect(onStart).toHaveBeenCalledTimes(1);
    expect(onWordBoundary.mock.calls).toEqual([[0, 5], [6, 5]]);
  ...
  it('advances word by word when a browser voice emits no boundary events', async () => {
    ...
    service.speak('English is his first language.', 0.85, { onWordBoundary });
    const utterance = speech.utterance();
    utterance.onstart?.({});
  ...
  it('stops fallback timing as soon as a native boundary appears', async () => {
    ...
    expect(onWordBoundary.mock.calls).toEqual([[0, 3], [4, 3]]);
  });
  ...
  it('ignores stale boundary events after a new utterance starts', () => {
    ...
    service.speak('First sentence', 0.85, { onWordBoundary: firstBoundary });
    const first = speech.utterance();
    service.speak('Second sentence', 0.85, { onWordBoundary: secondBoundary });
