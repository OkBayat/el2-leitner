...
describe('ListeningPracticeApiService', () => {
  it('uses the BBC lesson catalog and server-side attempt endpoints', async () => {
    const api = {
      get: vi.fn().mockResolvedValue({ provider: 'bbc_6_minute_english', lessons: [] }),
      post: vi.fn()
        .mockResolvedValueOnce({ attempt: { id: 'attempt-1' }, lesson: { id: 'lesson-1' } })
    ...
    await service.startBbcAttempt('climate change');
    ...
    expect(api.post).toHaveBeenNthCalledWith(
      1,
      '/api/listening/bbc/lessons/climate%20change/attempts',
    );
    expect(api.post).toHaveBeenNthCalledWith(
      2,
