...
The [hybrid Writing feedback design](WRITING_FEEDBACK.md) and the general
...
closed tasks, OpenAI feedback for open text, existing Kokoro for TTS, and
...
| Open Writing and short explanations | Evaluate `OpenAI gpt-5-nano` through server-side OpenAI | A candidate for evidence-based formative feedback, not a validated IELTS examiner |
| Provider operations | Managed OpenAI Responses API | Measure latency, token use, rate limits and quality without logging learner text |
...
| TTS | Retain Kokoro and the existing authenticated TTS boundary | The OpenAI model is text-only and does not replace Kokoro |
| Speaking now | Retain existing Vosk/Shadowing behavior | Repetition/word recognition is not independent Speaking assessment |
| Adaptive conversation | General JSON slide: browser PCM -> private Vosk -> OpenAI turn feedback/question -> existing Kokoro playback | Transcript-only feedback cannot assess pronunciation, fluency, or a full Speaking performance |
| Scores | Keep deterministic task results; use qualitative feedback for micro-exercises | Only full IELTS Writing modes may show a cautious band estimate |
...
| --- | --- | --- |
| WF-01 | Provider latency, cost and feedback-quality evaluation | Measured requests and reviewed examples support the enabled feature |
...
| WF-03 | Server-side OpenAI adapter and bounded worker | Authenticated backend access only, configured model identity, bounded resources and validated Responses API output |
...
| AC-02 | L0001 E09 as the first JSON consumer | Two accepted turns, validated OpenAI question generation, owned ephemeral Kokoro playback and no IELTS-band claim |

The course-production branch adds the WF-02/WF-03/WF-04 contracts, durable jobs,
server-side provider, existing-slide integration and backend configuration. It also implements
AC-01 and the optional AC-02 consumer using the same bounded text worker and
provider lease. WF-01's real provider measurements and actual
...
- Curriculum sections **8 and 13.3** retain original-attempt, repair, fresh-transfer
  and completion/mastery distinctions. AI feedback cannot mutate Leitner or award a pass.
