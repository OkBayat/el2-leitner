...
The actual IELTS Listening examination has FOUR parts, TEN questions in EACH part, and FORTY questions overall. Ten is the fixed per-part count, not a claimed minimum of thirteen. Recordings are heard once and questions follow the order of information in the recording. Each correct answer earns one mark.

Vocora uses short BBC recordings for supplementary IELTS-style practice. The product requirement is AT LEAST THIRTEEN scored questions in EACH test, not thirteen across an episode and not six with extra headings counted as questions. Default to thirteen unless the user requests more. This is an explicit Vocora extension, not the length of an official IELTS part. Do not claim that a BBC practice set is an official paper, an accredited IELTS test, a full examination simulation, or a calibrated IELTS band score. Playback controls remain practice aids; this content task must not silently remove them.
...
4. Select at least thirteen worthwhile information targets for each test. Include substantive material from the beginning, middle and later discussion. Do not pad the count with presenter names, the programme title, closing advertisements, unsupported trivia or repeated dictionary definitions.
...
Every affected test must have at least thirteen genuine scored questions, all four supported task types, sequential numbering, coherent instructions, valid answer limits, unique stable identities and verified forward-only source evidence. Preserve episode identities and test routes when editing existing content. Do not destroy completed attempt snapshots or learner progress.
