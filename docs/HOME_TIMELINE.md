...
`Home timeline integration` runs the focused domain/repository tests, rollback coverage, authenticated HTTP/MySQL 8.4 integration, and browser checks at 320, 390 and 1280 pixels. The browser checks cover gray future/today nodes, retained past colors, all six positions, dialog bounds, Escape/focus restoration, no completion writes on clicks, history scroll anchoring, retry and reload behavior. Browser layout uses explicit test responses; the separate MySQL job verifies persisted evidence and user isolation. The existing full Angular E2E workflow continues exercising real practice navigation through the new Home launchers.
